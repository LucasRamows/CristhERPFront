...
interface SupplierAllNotesHistorySheetProps {
  supplier: SupplierResponse | null;
  isOpen: boolean;
  onClose: () => void;
}

export function SupplierAllNotesHistorySheet({
  supplier,
  isOpen,
  onClose,
}: SupplierAllNotesHistorySheetProps) {
                    ...
                    >
                      ...
                      <ChevronRight
                        ...
                        className={`transition-transform duration-300 ${
                          selectedEntry === entry.id ? "rotate-90" : ""
                        }`}
                      />
                    </button>
                  </div>
                    ...
                    <div className="p-6 bg-muted/30 border-t border-border animate-in slide-in-from-top-4 duration-500">
                      <div className="space-y-4">
                        {entry.items.map((item) => (
                          <div
                            key={item.id}
                            className="flex justify-between items-center bg-background p-3 rounded-2xl border border-dashed border-border"
                          >
                              ...
                              <div className="flex flex-col">
                                <span className="font-black text-foreground text-xs uppercase tracking-tight">
                                  {item.ingredient?.name ||
                                    `Insumo #${item.ingredientId.slice(-4)}`}
                                </span>

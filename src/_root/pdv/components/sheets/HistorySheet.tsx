...
export function HistorySheet() {
  ...
  const handleDeleteOrder = async (orderId: string) => {
    ...
    try {
      ...
      // invalidateQueries always fetches fresh — no skipCache needed
      await refreshData();
  ...
  const unpaidOrders = openOrders.filter(
    (o) =>
      o.status?.toLowerCase() !== "paid" &&
      o.status?.toLowerCase() !== "canceled",
  );

  const mapOpenOrderToPdvEntity = (order: any): PdvEntity => ({
    id: order.id,
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent
        side="right"
        className="w-[90%] sm:max-w-112.5 p-0 flex flex-col h-full bg-white outline-none [&>button]:hidden"
      >
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setIsOpen(false);
                          handleEntityClick(mapOpenOrderToPdvEntity(order));
                        }}
                      >
                        <Pencil size={16} />
                        Alterar
